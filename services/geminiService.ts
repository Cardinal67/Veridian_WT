...
export const generateWorkoutPlan = async (request: PlanRequest) => {
    ...
    // Retrieve the API key from the pseudo-environment variables.
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey || apiKey === 'YOUR_GEMINI_API_KEY_HERE') {
        throw new Error("Gemini API key is not configured. Please add it to index.html to use AI features.");
    }
