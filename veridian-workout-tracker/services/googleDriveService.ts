...
// These values are pulled from the `window.process.env` object defined in `index.html`.
...
// Check if the API keys have been properly configured in `index.html`.
...
export const gapiCallWrapper = async <T>(apiCall: () => Promise<T>): Promise<T> => {
    ...
    if (!gapiInitialized) {
        throw new Error("Google API features are unavailable. Please ensure your Google Cloud Platform API Key is configured correctly in index.html.");
    }
